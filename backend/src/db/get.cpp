#include "db/get.h"
#include "db/config.h"
#include "managers/birthday_manager.h"
#include "db/connection_pool.h"
#include <iostream>

nlohmann::json Get::resultToJson(pqxx::result& r) {
    nlohmann::json result = nlohmann::json::array();
    for (const auto& row : r) {
        nlohmann::json obj = nlohmann::json::object();
        for (const auto& field : row) {
            if (!field.is_null()) {
                obj[field.name()] = field.as<std::string>();
            }
        }
        result.push_back(obj);
    }
    return result;
}

nlohmann::json Get::getOrganizations() {
    try {
        auto conn = ConnectionPool::getConnection();
        if (!conn) throw std::runtime_error("Failed to get connection");
        
        pqxx::work txn(*conn);
        static const std::string query = "SELECT * FROM organizations";
        
        std::string stmt_name = "get_orgs_" + std::to_string(reinterpret_cast<uintptr_t>(conn.get()));
        pqxx::result result;
        
        try {
            txn.conn().prepare(stmt_name, query);
            result = txn.exec_prepared(stmt_name);
            txn.exec0("DEALLOCATE " + stmt_name);
        } catch (...) {
            result = txn.exec(query);
        }

        auto json = resultToJson(result);
        txn.commit();
        ConnectionPool::releaseConnection(std::move(conn));
        return json;
    } catch (const std::exception& e) {
        return nlohmann::json::array();
    }
}

nlohmann::json Get::getDepartments() {
    try {
        auto conn = ConnectionPool::getConnection();
        if (!conn) throw std::runtime_error("Failed to get connection");
        
        pqxx::work txn(*conn);
        static const std::string query = 
            "SELECT d.*, o.name as organization_name "
            "FROM departments d "
            "LEFT JOIN LATERAL ("
            "    SELECT name FROM organizations WHERE id = d.organization_id"
            ") o ON true";

        std::string stmt_name = "get_depts_" + std::to_string(reinterpret_cast<uintptr_t>(conn.get()));
        pqxx::result result;
        
        try {
            txn.conn().prepare(stmt_name, query);
            result = txn.exec_prepared(stmt_name);
            txn.exec0("DEALLOCATE " + stmt_name);
        } catch (...) {
            result = txn.exec(query);
        }

        auto json = resultToJson(result);
        txn.commit();
        ConnectionPool::releaseConnection(std::move(conn));
        return json;
    } catch (const std::exception& e) {
        return nlohmann::json::array();
    }
}

nlohmann::json Get::getLocations() {
    try {
        pqxx::connection conn(Config::getConnectionString());
        pqxx::work txn(conn);
        pqxx::result r = txn.exec("SELECT * FROM locations");
        return resultToJson(r);
    } catch (std::exception const& e) {
        return nlohmann::json::array();
    }
}

nlohmann::json Get::getEmployees() {
    try {
        auto conn = ConnectionPool::getConnection();
        if (!conn || !conn->is_open()) {
            throw std::runtime_error("Failed to get database connection");
        }

        pqxx::work txn(*conn);
        
        static const std::string query = R"(
            SELECT 
                e.id, e.full_name, e.physical_person_name, 
                e.position, e.personnel_number, 
                COALESCE(TO_CHAR(e.dismissal_date, 'YYYY-MM-DD'), '') as dismissal_date,
                e.service, e.can_help_with, e.responsibilities,
                e.makes_decisions, e.is_dismissed, e.work_phone,
                COALESCE(TO_CHAR(e.birth_date, 'YYYY-MM-DD'), '') as birth_date,
                e.is_admin,
                o.name as organization,
                d.name as department,
                l.name as location,
                m.personnel_number as manager
            FROM employees e
            LEFT JOIN LATERAL (
                SELECT name FROM organizations WHERE id = e.organization_id
            ) o ON true
            LEFT JOIN LATERAL (
                SELECT name FROM departments WHERE id = e.department_id
            ) d ON true
            LEFT JOIN LATERAL (
                SELECT name FROM locations WHERE id = e.location_id
            ) l ON true
            LEFT JOIN LATERAL (
                SELECT personnel_number FROM employees WHERE id = e.manager_id
            ) m ON true
        )";

        std::string stmt_name = "get_employees_" + std::to_string(reinterpret_cast<uintptr_t>(conn.get()));
        
        pqxx::result result;
        try {
            txn.conn().prepare(stmt_name, query);
            result = txn.exec_prepared(stmt_name);
            txn.exec0("DEALLOCATE " + stmt_name);
        } catch (...) {
            result = txn.exec(query);
        }

        auto json = resultToJson(result);
        txn.commit();
        ConnectionPool::releaseConnection(std::move(conn));
        return json;

    } catch (const std::exception& e) {
        std::cerr << "Database error in getEmployees: " << e.what() << std::endl;
        return nlohmann::json::array();
    }
}

nlohmann::json Get::getEmployeeByPersonnelNumber(const std::string& personnel_number) {
    try {
        pqxx::connection conn(Config::getConnectionString());
        pqxx::work txn(conn);
        
        auto result = txn.exec_params(
            "SELECT * FROM employees WHERE personnel_number = $1",
            personnel_number
        );
        
        nlohmann::json json_array = nlohmann::json::array();
        
        for (const auto& row : result) {
            nlohmann::json obj;
            for (const auto& field : row) {
                if (!field.is_null()) {
                    obj[field.name()] = field.as<std::string>();
                }
            }
            json_array.push_back(obj);
        }
        
        return json_array;

    } catch (const std::exception& e) {
        return nlohmann::json::array();
    }
}

nlohmann::json Get::getNotifications() {
    try {
        pqxx::connection conn(Config::getConnectionString());
        pqxx::work txn(conn);
        pqxx::result r = txn.exec(
            "SELECT n.*, e.full_name as employee_name "
            "FROM notifications n "
            "LEFT JOIN employees e ON n.employee_id = e.id "
            "ORDER BY n.time DESC"
        );
        return resultToJson(r);
    } catch (std::exception const& e) {
        return nlohmann::json::array();
    }
}

nlohmann::json Get::getLinks() {
    try {
        auto conn = ConnectionPool::getConnection();
        if (!conn) throw std::runtime_error("Failed to get connection");

        pqxx::work txn(*conn);
        static const std::string query = R"(
            SELECT 
                id,
                name,
                description,
                url,
                CASE 
                    WHEN icon_data IS NOT NULL THEN encode(icon_data, 'base64')
                    ELSE NULL 
                END as icon_data,
                icon_type,
                icon_emoji
            FROM links 
            ORDER BY created_at DESC
        )";

        std::string stmt_name = "get_links_" + std::to_string(reinterpret_cast<uintptr_t>(conn.get()));
        pqxx::result result;

        try {
            txn.conn().prepare(stmt_name, query);
            result = txn.exec_prepared(stmt_name);
            txn.exec0("DEALLOCATE " + stmt_name);
        } catch (...) {
            result = txn.exec(query);
        }

        nlohmann::json json = nlohmann::json::array();
        for (const auto& row : result) {
            nlohmann::json portal = {
                {"id", row["id"].as<int>()},
                {"name", row["name"].as<std::string>()},
                {"description", row["description"].is_null() ? "" : row["description"].as<std::string>()},
                {"url", row["url"].as<std::string>()},
                {"icon_emoji", row["icon_emoji"].is_null() ? "" : row["icon_emoji"].as<std::string>()}
            };

            if (!row["icon_data"].is_null()) {
                portal["icon_data"] = row["icon_data"].as<std::string>();
                portal["icon_type"] = row["icon_type"].as<std::string>();
            }

            json.push_back(portal);
        }

        txn.commit();
        ConnectionPool::releaseConnection(std::move(conn));
        return json;
    } catch (const std::exception& e) {
        std::cerr << "Error in getLinks: " << e.what() << std::endl;
        return nlohmann::json::array();
    }
}

bool Get::checkIfLikedByEmployee(int newsId, int employeeId, pqxx::work& txn) {
    auto result = txn.exec_params(
        "SELECT 1 FROM news_likes WHERE news_id = $1 AND employee_id = $2",
        newsId, employeeId
    );
    return !result.empty();
}

nlohmann::json Get::getNewsWithDetails(int currentUserId) {
    try {
        auto conn = ConnectionPool::getConnection();
        if (!conn) throw std::runtime_error("Failed to get connection");

        pqxx::work txn(*conn);
        txn.exec0("SET TIME ZONE 'UTC'");

        static const std::string query = R"(
            WITH comment_details AS (
                SELECT 
                    nc.news_id,
                    jsonb_agg(
                        jsonb_build_object(
                            'id', nc.id,
                            'text', nc.text,
                            'author', e.full_name,
                            'created_at', TO_CHAR(nc.created_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"')
                        ) ORDER BY nc.created_at ASC
                    ) FILTER (WHERE nc.id IS NOT NULL) as comments
                FROM news_comments nc
                LEFT JOIN employees e ON nc.employee_id = e.id
                GROUP BY nc.news_id
            )
            SELECT 
                n.*,
                e.full_name as author_name,
                CASE 
                    WHEN n.image_data IS NOT NULL 
                    THEN encode(n.image_data, 'base64') 
                    ELSE NULL 
                END as image_data_base64,
                nl.employee_id IS NOT NULL as liked,
                (SELECT COUNT(*) FROM news_likes WHERE news_id = n.id) as likes_count,
                TO_CHAR(n.publication_time AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"') as publication_time,
                COALESCE(cd.comments, '[]'::jsonb) as comments
            FROM news n 
            LEFT JOIN employees e ON n.author_id = e.id
            LEFT JOIN news_likes nl ON n.id = nl.news_id AND nl.employee_id = $1
            LEFT JOIN comment_details cd ON n.id = cd.news_id
            ORDER BY 
                n.is_pinned DESC,
                n.pin_order ASC NULLS LAST,
                n.publication_time DESC
        )";

        std::string stmt_name = "get_news_" + std::to_string(reinterpret_cast<uintptr_t>(conn.get()));
        pqxx::result result;

        try {
            txn.conn().prepare(stmt_name, query);
            result = txn.exec_prepared(stmt_name, currentUserId);
            txn.exec0("DEALLOCATE " + stmt_name);
        } catch (...) {
            result = txn.exec_params(query, currentUserId);
        }

        nlohmann::json resultArray = nlohmann::json::array();
        
        for (const auto& row : result) {
            nlohmann::json newsItem = {
                {"id", row["id"].as<int>()},
                {"title", row["title"].as<std::string>()},
                {"content", row["content"].as<std::string>()},
                {"publication_time", row["publication_time"].as<std::string>()},
                {"author_name", row["author_name"].is_null() ? "" : row["author_name"].as<std::string>()},
                {"likes_count", row["likes_count"].as<int>()},
                {"liked", row["liked"].as<bool>()},
                {"is_pinned", row["is_pinned"].as<bool>()},
                {"comments", row["comments"].is_null() ? nlohmann::json::array() : nlohmann::json::parse(row["comments"].as<std::string>())},
                {"image_type", row["image_type"].is_null() ? "" : row["image_type"].as<std::string>()},
                {"image_data", row["image_data_base64"].is_null() ? "" : row["image_data_base64"].as<std::string>()}
            };
            resultArray.push_back(newsItem);
        }

        txn.commit();
        ConnectionPool::releaseConnection(std::move(conn));
        return resultArray;

    } catch (const std::exception& e) {
        std::cerr << "Error in getNewsWithDetails: " << e.what() << std::endl;
        return nlohmann::json::array();
    }
}

nlohmann::json Get::getNewsComments(int news_id) {
    try {
        pqxx::connection conn(Config::getConnectionString());
        pqxx::work txn(conn);
        
        pqxx::result r = txn.exec_params(R"(
            SELECT 
                nc.id,
                nc.text,
                nc.created_at,
                e.full_name as author_name,
                e.position as author_position
            FROM news_comments nc
            LEFT JOIN employees e ON nc.employee_id = e.id
            WHERE nc.news_id = $1
            ORDER BY nc.created_at ASC
        )", news_id);
        
        return resultToJson(r);
    } catch (std::exception const& e) {
        return nlohmann::json::array();
    }
}

nlohmann::json Get::getUpcomingBirthdays() {
    return BirthdayManager::getInstance().getUpcomingBirthdays();
}
